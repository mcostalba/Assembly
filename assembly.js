...
// A comma separated string like address0,label0,address1,label1,...
var LABELS = [];
var MYADDR;
...
function pn(reg) {
  ...
  if (!isNaN(id)) {
    if (id < 0)
      id = 0xFFFF + id + 1; // Two's complement
...
// Parse an absolute address out of a number or label
function pa(reg) {
...
// Parse an indirect absolute address
function pai(reg) {

  reg = reg.trim()
  if (reg.length > 2 && reg[0] == '(' && reg[reg.length - 1] == ')')
    return pa(reg.substring(1, reg.length - 1));
}

// Parse a relative offset out of a number or label
function po(reg) {
  
  reg = reg.trim()
  var addr = pa(reg)
  if (!isNaN(addr))
    return pn(String(addr - MYADDR))
}
